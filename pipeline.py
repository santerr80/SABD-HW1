...
class ExtractTar(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget(
            os.path.join(self.output_dir, f"{self.file_name}_tar_extracted.done")
        )
...
class ExtractGz(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget(
            os.path.join(self.output_dir, f"{self.file_name}_gz_extracted.done")
        )
...
class ProcessTxtFiles(luigi.Task):
    ...
    def process_file(self, file_path):
        ...
        for df_name, df in dfs.items():
            output_file = os.path.join(self.output_dir, f"{base_name}_{df_name}.tsv")
